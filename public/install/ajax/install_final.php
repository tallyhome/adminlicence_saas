 ...
 * Ce script crée toutes les tables nécessaires et les remplit avec les données initiales
 */
...
function logInfo($function, $data) {
    ...
    file_put_contents($logDir . '/installation_final.log', json_encode($logData, JSON_PRETTY_PRINT) . "\n\n", FILE_APPEND);
}
...
try {
    ...
    // 1. Supprimer toutes les tables existantes pour éviter les conflits
    $tables = [];
    ...
    if (!empty($tables)) {
        ...
        logInfo('tables_dropped', ['tables' => $tables]);
    }
    ...
    // 3. Exécuter les seeders si les migrations ont réussi
    $seedersRun = false;
    if ($migrationsRun) {
    ...
    // 5. Mettre à jour le fichier .env avec l'URL du projet
