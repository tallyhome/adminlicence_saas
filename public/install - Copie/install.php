...
function verifierLicence($cleSeriale, $domaine = null, $adresseIP = null) {
    ...
    // Données à envoyer avec les clés API
    $donnees = [
    ...
    // Configurer cURL avec des options de sécurité renforcées
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_POST, true);
    curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($donnees));
    curl_setopt($ch, CURLOPT_HTTPHEADER, [
        'Content-Type: application/json',
        'User-Agent: AdminLicence-Installer/1.0'
    ]);
    ...
    if ($reponse === false) {
        return [
            'valide' => false,
            'message' => 'Erreur de connexion au serveur de licences: ' . $erreur,
            'donnees' => null
        ];
    }
    ...
    foreach ($required_fields as $field) {
        if (!isset($resultat[$field])) {
            return [
                'valide' => false,
                'message' => 'Structure de réponse invalide: champ ' . $field . ' manquant',
                'donnees' => null
            ];
        }
    }
    ...
    if ($httpCode !== 200) {
        return [
            'valide' => false,
            'message' => 'Erreur du serveur de licences (Code: ' . $httpCode . ')',
            'donnees' => null
        ];
    }

    // Vérifier la structure des données de licence
    $required_data_fields = ['serial_key', 'status', 'expires_at', 'is_active', 'plan', 'client'];
    foreach ($required_data_fields as $field) {
        if (!isset($resultat['data'][$field])) {
            return [
                'valide' => false,
                'message' => 'Données de licence incomplètes: champ ' . $field . ' manquant',
                'donnees' => null
            ];
        }
    }
    ...
    if ($resultat['status'] !== 'success') {
        return [
            'valide' => false,
            'message' => $resultat['message'] ?? 'La clé de licence est invalide',
            'donnees' => null
        ];
    }

    if (!isset($resultat['data']) || !is_array($resultat['data']) || empty($resultat['data'])) {
        return [
            'valide' => false,
            'message' => 'Données de licence invalides ou manquantes',
            'donnees' => null
        ];
    }

    // Vérifier si la licence existe
    if (!isset($resultat['data']['serial_key'])) {
        return [
            'valide' => false,
    ...
    // Vérification du statut de la licence et des dates
    if (!isset($resultat['data']['status']) || !isset($resultat['data']['expires_at'])) {
        return [
            'valide' => false,
            'message' => 'Informations de licence incomplètes',
            'donnees' => null
        ];
    }

    // Vérifier la date d'expiration
    $expirationDate = strtotime($resultat['data']['expires_at']);
    ...
    switch ($resultat['data']['status']) {
        case 'suspended':
            return [
                'valide' => false,
                'message' => 'Cette licence est suspendue',
                'donnees' => null
            ];
        case 'expired':
            return [
        ...
        case 'inactive':
            return [
                'valide' => false,
                'message' => 'Cette licence est inactive',
                'donnees' => null
            ];
    }

    if (!isset($resultat['data']['is_active']) || $resultat['data']['is_active'] !== true) {
        return [
            'valide' => false,
            'message' => 'Cette licence n\'est pas active',
            'donnees' => null
        ];
    }
...
function t($key, $locale = 'fr') {
    $translations = [
        'fr' => [
            ...
            'licence_key_help' => 'Entrez votre clé de licence pour activer le logiciel',
            'licence_invalid' => 'Clé de licence invalide',
            'licence_valid' => 'Clé de licence valide',
        ],
        'en' => [
