<?php
/**
 * Script d'installation utilisant directement Artisan pour AdminLicence
 * Ce script utilise les commandes Artisan pour garantir que toutes les tables sont créées
 */
...
function logInfo($function, $data) {
    ...
    file_put_contents($logDir . '/installation_artisan.log', json_encode($logData, JSON_PRETTY_PRINT) . "\n\n", FILE_APPEND);
}
...
try {
    ...
    // 1. Exécuter les migrations Laravel avec migrate:fresh
    $migrationsOutput = executeCommand('php artisan migrate:fresh --force', $projectRoot);
    ...
    if (isset($_SESSION['admin_config']) && is_array($_SESSION['admin_config'])) {
        ...
        if ($adminsTableExists) {
            ...
            if (!$adminExists) {
                ...
                // Vérifier si la table admin_role existe
                $adminRoleTableExists = false;
                try {
                    $stmt = $pdo->query("SHOW TABLES LIKE 'admin_role'");
                    $adminRoleTableExists = $stmt->rowCount() > 0;
                } catch (PDOException $e) {
                    // La table n'existe pas
                }
                
                // Si la table admin_role existe, attribuer le rôle super-admin à l'administrateur
                if ($adminRoleTableExists) {
                    ...
                    if ($role) {
                        $roleId = $role['id'];
                        // Attribuer le rôle à l'administrateur
                        $stmt = $pdo->prepare("INSERT INTO admin_role (admin_id, role_id) VALUES (?, ?)");
