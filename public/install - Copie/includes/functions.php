...
function detectProjectType() {
    // Essayer d'abord le chemin standard (public/install/../..)
    ...
    // Vérifier si nous sommes dans un sous-répertoire public/ d'un projet Laravel
    if (basename($projectRoot) === 'public') {
        $parentRoot = realpath($projectRoot . '/..');
        
        // Vérifier si le répertoire parent est un projet Laravel
        if (file_exists($parentRoot . '/artisan') && is_dir($parentRoot . '/routes') && file_exists($parentRoot . '/routes/web.php')) {
            $projectRoot = $parentRoot;
            
            $logData = [
                'time' => date('Y-m-d H:i:s'),
                'function' => 'detectProjectType',
                'message' => 'Detection d\'un projet Laravel dans le repertoire parent',
                'adjusted_project_root' => $projectRoot
            ];
            file_put_contents($logDir . '/project_detection.log', json_encode($logData, JSON_PRETTY_PRINT) . "\n\n", FILE_APPEND);
        }
    }
    ...
    if (file_exists($projectRoot . '/artisan') && is_dir($projectRoot . '/routes') && file_exists($projectRoot . '/routes/web.php')) {
        ...
        $logData = [
            'time' => date('Y-m-d H:i:s'),
            'function' => 'detectProjectType',
            'message' => 'Projet Laravel détecté',
            'has_env' => $result['has_env'],
            'has_vendor' => $result['has_vendor']
        ];
        file_put_contents($logDir . '/project_detection.log', json_encode($logData, JSON_PRETTY_PRINT) . "\n\n", FILE_APPEND);
    }
